...
| Fabric | `fabric/` | Minecraft `26.2`, Fabric Loader `0.19.5`, Fabric API `0.160.0+26.2`, Loom `1.17-SNAPSHOT`, Gradle `9.5.1`, Java `25`, no mappings declaration | Shared JavaSound/DSP/config/profile/instrument runtime with Fabric lifecycle, tick telemetry, state-delta damage, movement/mining hooks, bounded loopback WebSocket output, and a keybound haptic test; sound interception, server relay, and custom config UI are not yet ported |
| NeoForge | `neoforge/` | Minecraft `26.2`, NeoForge `26.2.0.87`, Java `25` | Shared JavaSound/DSP/config/profile/instrument runtime with NeoForge client tick telemetry, state-delta damage, movement/mining hooks, and bounded loopback WebSocket output; Forge event parity and server relay are not yet ported |
...
The CI toolchains are intentionally separated: the retained Forge 1.20.1 project uses its Gradle 8.5 wrapper, Fabric 26.2 uses Gradle 9.5.1 with Loom 1.17-SNAPSHOT and no mappings declaration, and NeoForge 26.2 uses its own Gradle 9.2.1 wrapper.
