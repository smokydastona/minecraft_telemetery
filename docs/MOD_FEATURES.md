...
- NeoForge is a sibling project under `neoforge/`, using Minecraft `26.2`, NeoForge `26.2.0.87`, official NeoForge mappings, and Java 25.
...
- NeoForge 26.2 runtime validation is currently `BLOCKED — NeoForm artifact/input investigation required` at `:neoFormSetup`: `PatchBundleReader` cannot find `patches.lzma` in the resolved patch bundle. The resolved archive contents must distinguish a malformed artifact from NeoGradle selection/extraction behavior; no mod-source failure has been demonstrated.
...
GitHub Actions validates localization before building:
...
- CI also fails if any translation-target locale still looks like obvious English fallback content.
- English-variant and novelty locales are exempt from the translation-coverage gate, but they must still remain structurally synced with `lang/en_us.json`.
