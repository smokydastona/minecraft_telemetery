...
/**
 * Phase 3: per-bus routing editor for Sound Scape.
 *
 * This routes internal haptic buses (ui/danger/environmental/continuous/impact/modded)
 * to channels or groups in addition to category/override routing.
 */
public final class SpatialBusRoutingScreen extends Screen {
    private static final List<String> BUS_IDS = List.of(
        "ui",
        "danger",
        "environmental",
        "continuous",
    ...
    public SpatialBusRoutingScreen(Screen parent) {
        super(Component.translatable("bassshakertelemetry.spatial_bus.title"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        this.addRenderableWidget(new StringWidget(
            ...
            280,
            20,
            Objects.requireNonNull(Component.translatable("bassshakertelemetry.spatial_bus.title")),
            font
        ));
        ...
        settingsList.addSettingEntry(new LabelEntry("bassshakertelemetry.spatial_bus.section"));

        routingButtons.clear();
        for (String busId : BUS_IDS) {
            addBusRoutingEntry(contentWidth, rowH, busId);
        }
    ...
    private void addBusRoutingEntry(int contentWidth, int rowH, String busId) {
        ...
        String current = normalizeTargetId(BstConfig.get().soundScapeBusRouting.get(busId));
    ...
    private static String busLabelKey(String busId) {
        String b = (busId == null) ? "" : busId.trim().toLowerCase(Locale.ROOT);
        return switch (b) {
            case "ui" -> "bassshakertelemetry.spatial_bus.bus_ui";
            case "danger" -> "bassshakertelemetry.spatial_bus.bus_danger";
            case "environmental" -> "bassshakertelemetry.spatial_bus.bus_environmental";
            case "continuous" -> "bassshakertelemetry.spatial_bus.bus_continuous";
            case "impact" -> "bassshakertelemetry.spatial_bus.bus_impact";
            case "modded" -> "bassshakertelemetry.spatial_bus.bus_modded";
            default -> "bassshakertelemetry.spatial_bus.bus_unknown";
        };
    }
    ...
    private void onDone() {
        ...
        if (data.soundScapeBusRouting == null) {
            data.soundScapeBusRouting = new HashMap<>();
        }
        ...
        for (RoutingButton rb : routingButtons.values()) {
            ...
            String v = rb.current();
            if (v != null && !v.isBlank()) {
                data.soundScapeBusRouting.put(rb.busId, v);
            }
        }
    ...
    private record RoutingButton(String busId, List<String> options, int[] indexRef, Button button) {
        String current() {
            int idx = indexRef[0];
